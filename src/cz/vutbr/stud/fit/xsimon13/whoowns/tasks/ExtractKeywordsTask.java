...
public class ExtractKeywordsTask implements Task {
    @Override
    ...
    public void run(TeamAssignment teamAssignment, Jedis jedis, List<String> params) throws Exception {
        ...
        OwnershipStatistics ownershipStatistics = new OwnershipStatistics(jedis, ProcessChangelistsTask.TEAM_STATISTICS_NAME);
        int projectMonths = ownershipStatistics.getMonths();

        for (Team team : teamAssignment.getTeams()) {
            System.out.println("team " + team.getId() +
                               " (" + Long.toString(wordStatistics.getWordsInCategory(team.toEntityId())) + " words)");

            for (WordStatistics.Keyword keyword : wordStatistics.extractKeywordsForCategoryWithWeights(team.toEntityId(), maxKeywords, projectMonths))
